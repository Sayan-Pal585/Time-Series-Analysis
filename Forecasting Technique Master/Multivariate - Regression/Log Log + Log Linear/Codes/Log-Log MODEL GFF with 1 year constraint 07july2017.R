#################################################################################
################# code for Log Log regression model   ########################
################# code created on 11 July 2017        ###########################
#################################################################################
...
num_min_var=18                    ## number of minimum variables in the model
num_max_var=18                    ## number of maximum variables in the model                              
...
iteration<-4      ## number of the Log-Log model iteration , depending on new set of variables or data
...
name<-paste0("Log-Log iter-",iteration)
...
Model_Log.Log_fitted_actual<- data.frame(Model_iter_num=character(),
                                         Mod_eq = character(),
                                         No_of_indep = integer(),
...
Model_Log.Log_Summary <- data.frame(Model_iter_num=character(),
                                    Mod_eq = character(),
                                    No_of_indep = integer(),
                                    Trained_upto_year=integer(),
                                    Variables = character(),
...
for(i in 1:nrow(selected_layer)){       
  ...
  actual<-exp(model_data[which(year(model_data[,date_var])<=global_end),dep_var] )
  for(j in 1:cross_val_prd){
    ...
    if(i==1 & j==1){
      Model_Log.Log_fitted_actual<-rbind(Model_Log.Log_fitted_actual,fit_fitted_actual)
    }else{
      Model_Log.Log_fitted_actual<-rbind.fill(Model_Log.Log_fitted_actual,fit_fitted_actual)
    }
    ...
    if(i==1 & j==1){
      Model_Log.Log_Summary<-rbind(Model_Log.Log_Summary,summary)
    }else{
      Model_Log.Log_Summary<-rbind.fill(Model_Log.Log_Summary,summary) 
    }
...
write.csv(Model_Log.Log_fitted_actual,paste0(name," fitted values for ",country," run on ",date,".csv"))
write.csv(Model_Log.Log_Summary,paste0(name," summary for ",country," run on ",date,".csv"))
...
####### code to summarize the model iterations and finding best iteration #####
Model_Log.Log_fitted_actual$year<-year(Model_Log.Log_fitted_actual[,date_var])
Model_Log.Log_fitted_actual$out.flag<-(Model_Log.Log_fitted_actual$year)-(Model_Log.Log_fitted_actual$Trained_upto_year)
outsample<-Model_Log.Log_fitted_actual[which(Model_Log.Log_fitted_actual$out.flag>=0),]
