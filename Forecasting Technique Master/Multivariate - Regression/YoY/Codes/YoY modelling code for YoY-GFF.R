...
dep_var="Beer.Consumption.Khl_yoy"   ## name of the dependent variable as in raw data
orig_var<-"Beer.Consumption.Khl"
date_var="Date"                  ## name of the date variable as in raw data
num_min_var=12                    ## number of minimum variables in the model
num_max_var=14                    ## number of maximum variables in the model                              
global_start=2013                ## year from which yoy data is available
...
#raw_data<-yoy.data
yoy_calc<-raw_data[,c(date_var,orig_var)]
colnames(yoy_calc)[2]<-"actual"
yoy_calc$actual_lag12<-c(rep(NA,12),yoy_calc[1:(nrow(yoy_calc)-12),"actual"])
################## creating automated names for files to be saved ############
##############################################################################

name<-paste0("YoY regression iter-",iteration)
...
##    NOTE:header of the shortlisted variables (only independent variables)
...
model_data<-model_data[which(year(model_data[,date_var])>=global_start),]
...
Model_YoY_fitted_actual<- data.frame(Model_iter_num=character(),
                                     Mod_eq = character(),
                                     No_of_indep = integer(),
...
Model_YoY_Summary <- data.frame(Model_iter_num=character(),
                                Mod_eq = character(),
                                No_of_indep = integer(),
                                Trained_upto_year=integer(),
                                Variables = character(),
...
for(i in 1:nrow(selected_layer)){       
  ...
  for(j in 1:cross_val_prd){
    ...
    fit_val<-c(as.vector(predict(fit,train)),as.vector(predict(fit,test)))
    fit_val_low.95<-c(as.vector(predict(fit,train, interval="predict")[,2]),
                      as.vector(predict(fit,test, interval="predict")[,2]))
    fit_val_upr.95<-c(as.vector(predict(fit,train, interval="predict")[,3]),
                      as.vector(predict(fit,test, interval="predict")[,3]))
    
    fit_val_low.90<-c(as.vector(predict(fit,train, interval="predict",level=0.90)[,2]),
                      as.vector(predict(fit,test, interval="predict",level=0.90)[,2]))
    fit_val_upr.90<-c(as.vector(predict(fit,train, interval="predict",level=0.90)[,3]),
                      as.vector(predict(fit,test, interval="predict",level=0.90)[,3]))
    
    fit_val<-(1+fit_val)*yoy_calc$actual[1:length(fit_val)]
    fit_val_low.95<-(1+fit_val_low.95)*yoy_calc$actual[1:length(fit_val)]
    fit_val_upr.95<-(1+fit_val_upr.95)*yoy_calc$actual[1:length(fit_val)]
    fit_val_low.90<-(1+fit_val_low.90)*yoy_calc$actual[1:length(fit_val)]
    fit_val_upr.90<-(1+fit_val_upr.90)*yoy_calc$actual[1:length(fit_val)]
    actual<-yoy_calc$actual[1:length(fit_val)]
    ...
    if(i==1 & j==1){
      Model_YoY_fitted_actual<-rbind(Model_YoY_fitted_actual,fit_fitted_actual)
    }else{
      Model_YoY_fitted_actual<-rbind.fill(Model_YoY_fitted_actual,fit_fitted_actual)
    }
    ...
    summary<-summary[,c(2,3,4,5,1,6:ncol(summary))]
    
    if(i==1 & j==1){
      Model_YoY_Summary<-rbind(Model_YoY_Summary,summary)
    }else{
      Model_YoY_Summary<-rbind.fill(Model_YoY_Summary,summary) 
    }
...
write.csv(Model_YoY_fitted_actual,paste0(name," fitted values for ",country," run on ",date,".csv"))
write.csv(Model_YoY_Summary,paste0(name," summary for ",country," run on ",date,".csv"))
...
Model_YoY_fitted_actual$year<-year(Model_YoY_fitted_actual[,date_var])
Model_YoY_fitted_actual$out.flag<-(Model_YoY_fitted_actual$year)-(Model_YoY_fitted_actual$Trained_upto_year)
outsample<-Model_YoY_fitted_actual[which(Model_YoY_fitted_actual$out.flag>=0),]
