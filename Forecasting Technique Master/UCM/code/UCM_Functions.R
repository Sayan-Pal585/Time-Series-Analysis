...
ucm.model.results <- function(training_data, 
                              test_data, 
                              trn.period, 
                              tst.period, 
                              freq, 
  ...
  ucm.model <- 0
  ucm.base = 5
  print(paste("UCM log base taken as", ucm.base))
  ucm.ts <- ts(log(training_data, base = ucm.base), frequency = freq)
  ucm.ts <- data.frame(ucm.ts)
  colnames(ucm.ts) <- "DISTRIBUTION"
  
  # Build UCM Model
  ucm.model <- ucm(formula = DISTRIBUTION~0,
                   data = ucm.ts,
                   level = TRUE,
                   slope = TRUE,
                   season = TRUE,
  ...
  # Forecast Values
  ucm.fitted.pred <- ucm.base ^ predict(ucm.model$model)
  ucm.prediction <- predict(ucm.model$model, n.ahead = (forecast.duration + tst.period))
  ucm.pred <- ucm.base ^ ucm.prediction
  ucm.forecast.series <- rbind(ucm.fitted.pred, ucm.pred)
  
  # Build the series (insample, outsampl, forecast)
  if (tst.period != 0) {
    ucm.series <- data.frame(cbind(ucm.forecast.series,
  ...
  colnames(ucm.series) <- c("PREDICTED","ACTUAL")
  
  ucm.series$ERROR <- ucm.series$PREDICTED - ucm.series$ACTUAL
  ucm.series$APE <- abs(ucm.series$ERROR)/ucm.series$ACTUAL
  
  ucm.insample <- mean(ucm.series[((freq * in.err.skip.dur) + 1):trn.period, ]$APE, na.rm = T)
  
  if (tst.period != 0) {
    ucm.outsample <- mean(ucm.series[trn.period + 1: (trn.period + tst.period), ]$APE, na.rm = T)
  } else {
    ucm.outsample <- 0
  }
  
  ucm.overall <- mean(ucm.series[1:(trn.period + tst.period), ]$APE, na.rm = T)
  
  
  ucm.MAPE <- c(rep(NA, 3))
  if (abs(ucm.insample - ucm.outsample) < error_threshold)
    ucm.MAPE <- c(ucm.insample,
                  ucm.outsample,
                  ucm.overall)
  ucm.MAPE <- as.data.frame(ucm.MAPE)
  row.names(ucm.MAPE) <- c("InSample","OutSample","OverallMape")
  ...
  write.csv(ucm.MAPE, paste(out_path,"UCM MAPE.csv",sep="/"))
  write.csv(ucm.series, paste(out_path,"UCM Series.csv",sep="/"))
  
  pdf(paste(out_path,"- UCM Actual vs Predicted.pdf",sep="/"))
  
  plot(ucm.series$PREDICTED, col = "red", main = "UCM - Actual (blue) vs Predicted (red)", type = "l", xaxt = "n")
  lines(ucm.series$ACTUAL, col = "blue", type = "l")
  dev.off()
  
  return(list(ucm.series, ucm.MAPE))
}
