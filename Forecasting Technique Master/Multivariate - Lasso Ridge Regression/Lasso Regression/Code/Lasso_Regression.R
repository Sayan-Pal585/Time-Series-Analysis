...
#RUN "VARIABLE SELECTION - LASSO REGRESSION" MODULE FOR SHORTLISTED
...
for(i in 1:nrow(selected_layer)){       
  ...
  for(j in 1:cross_val_prd){
    ...
    lasso_x <- data.matrix(train[,c(2:ncol(train))])
    lasso_y <- train$beer_volume
    ...
    getcoeff <- function(method = "method"){
      ...
      names(lasso_coeff) <- c("Variables", "Coefficient")
      lasso_coeff$abscoeff <- abs(lasso_coeff$Coefficient)
    ...
    lasso_train <- data.matrix(train[,c(2:ncol(train))])
    pred_train <- predict(fit.lasso, as.matrix(lasso_train))
    
    lasso_test <- data.matrix(test[,c(2:ncol(test))])
    pred_test <- predict(fit.lasso, as.matrix(lasso_test))
    ...
    lasso_coeff1se
    summary<-data.frame(Variables=lasso_coeff1se[,1])
    ...
    summary$Estimate = lasso_coeff1se[,2]
